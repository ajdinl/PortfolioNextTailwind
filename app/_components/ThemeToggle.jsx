    ...
    <div className='flex items-center justify-end gap-2 mb-6 text-base lg:text-sm dark:bg-[#252525] bg-mybackground'>
      <button
        ...
        className={`
                    ...
                    ${isReady ? 'cursor-pointer transition-colors' : 'cursor-default'}
                `}
      ...
      >
        ...
        >
          {isDarkMode ? (
            <MoonIcon className='w-4 h-4 md:w-5 md:h-5 text-black hover:text-gray-700 transition-colors duration-200' />
          ) : (
            <SunIcon className='w-4 h-4 md:w-5 md:h-5 text-yellow-500 hover:text-yellow-400 transition-colors duration-200' />
          )}
        </div>
      </button>
    </div>
